...
function Table({rows}:{rows:any[]}) {
 ...
 return <div className="table-wrap"><table><thead><tr>{cols.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{rows.slice(0,100).map((r,i)=><tr key={i}>{cols.map(c=><td key={c}>{r[c] == null ? "—" : typeof r[c]==="number" ? r[c].toLocaleString("es-CL") : String(r[c])}</td>)}</tr>)}</tbody></table></div>
}
